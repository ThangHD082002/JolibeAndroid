...
public class FragmentOrderPrepare extends Fragment implements RecycleViewOrderAdapter.ItemListener{
...
//                            listItemOrder = new ArrayList<>(order.getListItemOrder());
                            for(int i = 1; i < order.getListItemOrder().size(); i++){
                                if(order.getListItemOrder().get(i).getState().equals("chuẩn bị")){
...
//                            Toast.makeText(getContext(), "Enter Order Success", Toast.LENGTH_SHORT).show();
                        ...
                        adapter.setItemListener(FragmentOrderPrepare.this);
                    }
                } else {

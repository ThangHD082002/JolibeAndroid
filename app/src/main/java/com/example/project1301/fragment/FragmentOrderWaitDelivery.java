...
public class FragmentOrderWaitDelivery extends Fragment implements RecycleViewOrderAdapter.ItemListener{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_wait_delivery, container, false);
...
//                            listItemOrder = new ArrayList<>(order.getListItemOrder());
                            for(int i = 1; i < order.getListItemOrder().size(); i++){
                                if(order.getListItemOrder().get(i).getState().equals("đang giao")){
...
//                            Toast.makeText(getContext(), "Enter Order Success", Toast.LENGTH_SHORT).show();
                        ...
                        adapter.setItemListener(FragmentOrderWaitDelivery.this);
                    }
                } else {
